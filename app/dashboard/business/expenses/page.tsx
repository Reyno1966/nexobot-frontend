...
export default function ExpensesPage() {
  ...
  function buildExportData() {
    const columns = [
      ...
      { header: "Proveedor",    key: "proveedor",   width: 18, align: "left"   as const },
      { header: "Monto (USD)",  key: "monto",       width: 14, align: "right"  as const },
      { header: "Notas",        key: "notas",       width: 24, align: "left"   as const },
    ];
    ...
    const totalAmt = filtered.reduce((sum, e) => sum + Number(e.amount), 0);
    const totals = {
      fecha:       "",
      descripcion: "TOTAL",
      categoria:   filterCat !== "Todas" ? filterCat : "",
      proveedor:   `${filtered.length} gasto${filtered.length !== 1 ? "s" : ""}`,
      monto:       totalAmt.toFixed(2),
      notas:       "",
    };
    const catLabel = filterCat === "Todas" ? "Todas las categorías" : filterCat;
    const subtitle = `Período: ${period.label} · Filtro: ${catLabel} · Generado: ${new Date().toLocaleDateString("es", { day: "2-digit", month: "long", year: "numeric" })}`;
    const filename = `gastos_${period.year}-${period.month}`;
  ...
  async function handleExport(type: "xlsx" | "pdf") {
    ...
    const opts = { filename, title: "Reporte de Gastos", subtitle, columns, rows, totals };
    try {
